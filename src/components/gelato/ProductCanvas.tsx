    ...
    <div className={`relative w-full max-w-lg mx-auto ${className}`}>
              ...
              {gelatoGeneratedPreviewUrls!.length > 1 && (
                <>
                  <button
                    ...
                    className="absolute left-3 top-1/2 -translate-y-1/2 bg-white/90 hover:bg-white text-gray-700 hover:text-gray-900 p-3 rounded-full shadow-lg transition-all duration-200 z-10 backdrop-blur-sm border border-gray-200/50"
                  ...
                  <button
                    ...
                    className="absolute right-3 top-1/2 -translate-y-1/2 bg-white/90 hover:bg-white text-gray-700 hover:text-gray-900 p-3 rounded-full shadow-lg transition-all duration-200 z-10 backdrop-blur-sm border border-gray-200/50"
                  ...
                  <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2 z-10 bg-black/20 backdrop-blur-sm rounded-full px-3 py-2">
                    {gelatoGeneratedPreviewUrls!.map((_, index) => (
                      <button
                        ...
                        aria-label={`Ver mockup ${index + 1} de ${gelatoGeneratedPreviewUrls!.length}`}
                      />
                    ))}
                  </div>
                  ...
                  <div className="absolute top-4 right-4 bg-black/60 text-white text-xs px-2 py-1 rounded-full backdrop-blur-sm z-10">
                    {currentPreviewIndex + 1} / {gelatoGeneratedPreviewUrls!.length}
                  </div>
                </>
              )}
            </>
      ...
      {shouldUseGelatoMockups && (
        <div className="mt-3 text-center space-y-2">
          <div className="inline-flex items-center gap-2 bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 rounded-lg px-4 py-2 text-sm text-green-800">
            <span>✨</span>
            <span className="font-medium">Preview profissional gerado</span>
          </div>
          
          {/* Instruções do carrossel se houver múltiplos mockups */}
          {gelatoGeneratedPreviewUrls!.length > 1 && (
            <div className="text-xs text-gray-600 bg-gray-50 rounded-md px-3 py-2 inline-block">
              📸 {gelatoGeneratedPreviewUrls!.length} vistas diferentes • Use as setas ou clique nos pontos para navegar
            </div>
          )}
        </div>
      )}
