...
const Projects = () => {
  ...
  return (
    <section id="projects" className="section-padding relative overflow-hidden">
      ...
      <div className="absolute top-32 left-32 w-80 h-80 bg-gradient-to-br from-purple-500/10 to-pink-500/10 rounded-full blur-3xl animate-pulse"></div>
      <div className="absolute bottom-32 right-32 w-72 h-72 bg-gradient-to-br from-blue-500/10 to-cyan-500/10 rounded-full blur-3xl animate-pulse" style={{animationDelay: '1s'}}></div>
      
      <div className="container mx-auto px-4 relative z-10">
        <div className="text-center mb-16">
          <h2 className="section-title flex items-center justify-center gap-3">
            <Sparkles className="text-cosmic-purple animate-pulse" size={48} />
            Featured Projects
            <Zap className="text-cosmic-blue animate-pulse" size={32} />
          </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-7xl mx-auto">
          {projects.map((project, index) => (
            <div 
              key={project.id}
              className="group relative overflow-hidden rounded-2xl transition-all duration-500 transform hover:-translate-y-4 hover:scale-105"
            ...
            >
              ...
              <div 
                className="relative h-full p-1 rounded-2xl"
                style={{
                  background: 'linear-gradient(145deg, rgba(255,255,255,0.1) 0%, rgba(255,255,255,0.05) 100%)',
                  boxShadow: hoveredId === project.id 
                    ? '0 25px 50px rgba(139, 92, 246, 0.3), 0 0 0 1px rgba(255,255,255,0.2)' 
                    : '0 10px 30px rgba(0,0,0,0.3), 0 0 0 1px rgba(255,255,255,0.1)'
                }}
              >
                <div className="relative h-full bg-cosmic-black/40 backdrop-blur-xl rounded-xl overflow-hidden">
                  ...
                  <div className="relative h-56 overflow-hidden">
                    ...
                    <div className={`absolute inset-0 bg-cosmic-blue/20 transition-opacity duration-300 ${
                      ...
                      <Eye className="text-white text-4xl animate-pulse" />
                    </div>
                  </div>
                  
                  {/* Project content */}
                  <div className="p-6 space-y-4">
                    <h3 className="text-xl font-bold transition-all duration-300 group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-cosmic-blue group-hover:to-cosmic-purple">
                      {project.title}
                    </h3>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {project.tags.map((tag, tagIndex) => (
                        <span 
                          key={tagIndex}
                          className="text-xs bg-gradient-to-r from-cosmic-blue/20 to-cosmic-purple/20 text-cosmic-blue px-3 py-1 rounded-full border border-cosmic-blue/30 group-hover:scale-105 transition-transform duration-300 backdrop-blur-sm"
                        >
                          {tag}
                        </span>
                      ))}
              ...
              <div className={`absolute inset-0 rounded-2xl bg-gradient-to-r from-cosmic-blue via-cosmic-purple to-cosmic-pink opacity-0 group-hover:opacity-100 transition-opacity duration-300 -z-10 blur-md ${
