...
export const ProductGuarantees: React.FC<ProductGuaranteesProps> = ({
  ...
  return (
    <div className={`grid grid-cols-2 gap-3 sm:gap-4 w-full ${className}`}>
      {guarantees.map((guarantee, index) => (
        <div 
          key={index}
          className="group p-4 sm:p-4 bg-gradient-to-br from-ghibli-cream/40 to-ghibli-cream/20 rounded-xl hover:from-ghibli-cream/60 hover:to-ghibli-cream/30 transition-all duration-300 text-center border border-ghibli-sand/30 min-h-[80px] flex flex-col items-center justify-center"
        >
