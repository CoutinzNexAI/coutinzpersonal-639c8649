    ...
    export default async function handler(
      ...
      try {
        ...
        // Para este teste, vamos forçar o productId da t-shirt para que a chamada Get Template seja feita para ela
        const testProductId = 'tshirt_unisex_s_white_crewneck_premium'; // <-- USA O ID DA T-SHIRT AQUI
        console.log("📋 Dados recebidos (com productId forçado para teste Get Template):", { productId: testProductId, userImageUrl: userImageUrl?.substring(0, 50) + '...', userId });
        ...
        // Buscar produto no nosso mapeamento (usando o ID de teste da t-shirt)
        const product = getGelatoProduct(testProductId); // <-- USA O ID DA T-SHIRT AQUI
        if (!product) {
          console.log("❌ ERRO: Produto não encontrado no mapeamento:", testProductId);
          return res.status(404).json({
            success: false,
            error: `Product not found in mapping: ${testProductId}`
          });
        }

        console.log(`✅ Produto encontrado no mapeamento: ${product.name} (${product.productUid})`);

        // --- CÓDIGO TEMPORÁRIO PARA OBTER DETALHES DO TEMPLATE DA T-SHIRT ---
        ...
        try {
          ...
          // REMOVER ESTE BLOCO APÓS OBTERES OS IDs E NOMES NECESSÁRIOS E ATUALIZARES gelatoProducts.ts
        } catch (templateError) {
          console.error('❌ ERRO ao buscar detalhes do template Gelato da T-shirt:', templateError);
          // Não vamos parar o processo aqui, mas registar o erro
          // return res.status(500).json({ success: false, error: 'Failed to fetch t-shirt template details' });
        }
        // --- FIM DO CÓDIGO TEMPORÁRIO ---


        // O RESTO DO TEU FLUXO NORMAL (GERAR FICHEIRO DE IMPRESSÃO, CRIAR PRODUTO NA LOJA, CRIAR DRAFT ORDER)
        // CONTINUARÁ A USAR O PRODUCT ID ORIGINAL (OU SEJA, O CANVAS POR AGORA).
        // ISTO É APENAS PARA OBTER OS DADOS DO TEMPLATE DA T-SHIRT.
        // QUANDO TIVERES OS DADOS, VAIS ATUALIZAR O product.gelatoTemplateId, product.templateVariantId e product.printArea
        // NO TEU `gelatoProducts.ts` E DEPOIS REMOVER ESTE BLOCO TEMPORÁRIO E USAR O productID REAL.


        // PASSO 1: Gerar ficheiro de impressão de alta resolução (ainda para o Canvas original se productId não for alterado acima)
        ...
        const printFileResponse = await fetch(`${req.headers.origin || 'http://localhost:3000'}/api/gelato/generate-print-file`, {
          ...
          body: JSON.stringify({
            ...
            productId: productId, // Aqui vai usar o productId original (do request)
            userId: userId
          })
        });
        ...
        // PASSO 1.5: Criar produto na loja Gelato (ainda para o Canvas original se productId não for alterado acima)
