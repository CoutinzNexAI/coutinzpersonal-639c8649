...
async function processImage(jobId: string, jobData: JobData) {
  ...
  try {
    ...
    if (axios.isAxiosError(error)) {
        ...
        console.error(`[processImage: ${jobId}] Non-Axios ERROR: ${errorMessage}`, { errorObject: castError, stack: castError.stack });
        if (errorMessage.includes('connectivity test failed')) finalStatus = 'failed_connectivity_test';
