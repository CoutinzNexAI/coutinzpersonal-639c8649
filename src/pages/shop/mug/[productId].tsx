...
const MugDetailPage: React.FC = () => {
  ...
  // Estado específico para seleção de variante (tamanho da caneca)
  ...
  const handleAddToCart = async () => {
    ...
    // Validação específica para canecas - variante selecionada
    ...
    try {
      ...
      console.log('🛒 Adicionando caneca ao carrinho com valores:', {
      ...
      const cartItem = CartService.addToCart({
        ...
        quantity: 1,
        customizations: {
          size: product.variants?.find(v => v.id === selectedPrintifyVariantId)?.title || 'Tamanho não encontrado'
        },
      ...
      console.log('✅ Caneca adicionada ao carrinho:', cartItem);
  ...
  const handleImageAdjustmentChange = (adjustments: Partial<ImageAdjustments>) => {
    if (imageAdjustments) {
      // Manter X e Y sempre centrados (0.5) - não permitir alteração para canecas
      setImageAdjustments({ 
        ...imageAdjustments, 
        ...adjustments, 
  ...
  return (
    <>
      <Head>
        ...
        <meta name="description" content={`Personalize a sua ${product.name} com as suas criações AI. Caneca de cerâmica durável com impressão de alta qualidade.`} />
      </Head>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            >
              ...
              <div className="prose prose-sm text-ghibli-earth/80">
                <p>
                  Transforme as suas criações AI numa caneca personalizada única! Feita com cerâmica 
                  durável e impressão por sublimação de alta qualidade. Perfeita para o café da manhã 
              ...
              {product.variants && product.variants.length > 1 && (
                <div className="mb-4">
                  <label htmlFor="size-select" className="block text-sm font-medium text-ghibli-earth mb-2">
                    Escolha o Tamanho:
                  </label>
                  <Select
                  ...
                  >
                    <SelectTrigger id="size-select" className="w-full bg-white/70 text-ghibli-earth border-ghibli-sand">
                      <SelectValue placeholder="Selecione um tamanho">
                        {product.variants?.find(v => v.id === selectedPrintifyVariantId)?.title || 'Selecione um tamanho'}
                      </SelectValue>
                    </SelectTrigger>
              ...
              <div className="bg-white/50 rounded-lg p-3 space-y-2">
                ...
                <div className="space-y-1 text-xs">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">Cerâmica Durável</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-ghibli-earth/60">Impressão:</span>
                    <span className="font-medium">Sublimação HD</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-ghibli-earth/60">Qualidade:</span>
                    <span className="font-medium">Resistente a lavagem</span>
                  </div>
                  <div className="flex justify-between">
...
// Geração estática dos paths para produtos de canecas
