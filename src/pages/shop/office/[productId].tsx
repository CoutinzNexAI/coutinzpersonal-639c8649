...
const OfficeDetailPage: React.FC<OfficeDetailPageProps> = ({ product: initialProduct }) => {
  ...
  // Estado específico para seleção de variante (mouse pad só tem uma variante)
  ...
  useEffect(() => {
    if (!initialProduct && typeof productId === 'string') {
      ...
      if (foundProduct && foundProduct.category === 'office') {
  ...
  const handleAddToCart = async () => {
    ...
    // Usar a variante selecionada (que é automática para mouse pad)
    ...
    try {
      ...
      console.log('🛒 Adicionando mouse pad ao carrinho com valores:', {
      ...
      const cartItem = CartService.addToCart({
        ...
        quantity: 1,
        customizations: {
          size: product.variants?.find(v => v.id === selectedPrintifyVariantId)?.title || '9" × 7.5"'
        },
      ...
      console.log('✅ Mouse pad adicionado ao carrinho:', cartItem);
      ...
      toast.success('Mouse pad adicionado ao carrinho!', {
  ...
  const handleImageAdjustmentChange = (adjustments: Partial<ImageAdjustments>) => {
    if (imageAdjustments) {
      // Manter X e Y sempre centrados (0.5) - não permitir alteração para mouse pads
      setImageAdjustments({ 
        ...imageAdjustments, 
        ...adjustments, 
  ...
  return (
    <>
      <Head>
        ...
        <meta name="description" content={`Personalize o seu ${product.name} com as suas criações AI. Mouse pad antiderrapante com impressão de alta qualidade.`} />
      </Head>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            >
              ...
              <div className="prose prose-sm text-ghibli-earth/80">
                <p>
                  Transforme o seu espaço de trabalho! Este mouse pad personalizado é feito com 
                  material antiderrapante e superfície lisa para precisão máxima. Impressão de 
                  alta qualidade que não desbota.
                </p>
              </div>
              ...
              <div className="bg-white/50 rounded-lg p-3 space-y-2">
                ...
                <div className="space-y-1 text-xs">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">9" × 7.5" (23×19cm)</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-ghibli-earth/60">Material:</span>
                    <span className="font-medium">Tecido + Borracha</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-ghibli-earth/60">Base:</span>
                    <span className="font-medium">Antiderrapante</span>
                  </div>
                  <div className="flex justify-between">
                ...
                {!userInfo && (
                  <div className="text-center p-4 bg-blue-50 rounded-lg">
                    <p className="text-sm text-blue-800 mb-2">
                      Faça login para personalizar este mouse pad com as suas criações AI
                    </p>
                    <Button
                      onClick={() => router.push('/')}
                      variant="outline"
...
export const getStaticPaths: GetStaticPaths = async () => {
  ...
  const paths = Object.keys(officeProducts).map((productId) => ({
    params: { productId }
  }));
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  ...
  if (!product || product.category !== 'office') {
    return {
      notFound: true
    };
  }
