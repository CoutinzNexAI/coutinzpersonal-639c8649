...
const BagDetailPage: React.FC<BagDetailPageProps> = ({ product: initialProduct }) => {
  ...
  // Estado específico para seleção de variante (cor do saco)
  ...
  const handleAddToCart = async () => {
    ...
    // Validação específica para sacos - variante selecionada
    ...
    try {
      ...
      console.log('🛒 Adicionando saco ao carrinho com valores:', {
      ...
      const cartItem = CartService.addToCart({
        ...
        quantity: 1,
        customizations: {
          color: product.variants?.find(v => v.id === selectedPrintifyVariantId)?.title || 'Cor não encontrada'
        },
      ...
      console.log('✅ Saco adicionado ao carrinho:', cartItem);
  ...
  const handleImageAdjustmentChange = (adjustments: Partial<ImageAdjustments>) => {
    if (imageAdjustments) {
      // Manter X e Y sempre centrados (0.5) - não permitir alteração para sacos
      setImageAdjustments({ 
        ...imageAdjustments, 
        ...adjustments, 
  ...
  return (
    <>
      <Head>
        ...
        <meta name="description" content={`Personalize o seu ${product.name} com as suas criações AI. Saco de algodão resistente e ecológico com impressão de alta qualidade.`} />
      </Head>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            >
              ...
              <div className="prose prose-sm text-ghibli-earth/80">
                <p>
                  Leve as suas criações AI para todo o lado! Este saco tote bag é feito com 
                  algodão resistente e ecológico. Perfeito para compras, trabalho ou lazer. 
                  Design único com impressão duradoura.
                </p>
              </div>
              ...
              {product.variants && product.variants.length > 1 && (
                <div className="mb-4">
                  <label htmlFor="color-select" className="block text-sm font-medium text-ghibli-earth mb-2">
                    Escolha a Cor:
                  </label>
                  <Select
                  ...
                  >
                    <SelectTrigger id="color-select" className="w-full bg-white/70 text-ghibli-earth border-ghibli-sand">
                      <SelectValue placeholder="Selecione uma cor">
                        {product.variants?.find(v => v.id === selectedPrintifyVariantId)?.title || 'Selecione uma cor'}
                      </SelectValue>
                    </SelectTrigger>
              ...
              <div className="bg-white/50 rounded-lg p-3 space-y-2">
                ...
                <div className="space-y-1 text-xs">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">Serigrafia HD</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-ghibli-earth/60">Sustentável:</span>
                ...
                {!userInfo && (
                  <div className="text-center p-4 bg-blue-50 rounded-lg">
                    <p className="text-sm text-blue-800 mb-2">
                      Faça login para personalizar este saco com as suas criações AI
                    </p>
                    <Button
                      onClick={() => router.push('/')}
                      variant="outline"
...
// Geração estática dos paths para produtos de sacos
